...
const generatePassword = () => {
    ...
    if (settings.value.includeLowercase) charset += 'abcdefghijklmnopqrstuvwxyz';
    if (settings.value.includeUppercase) charset += 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    if (settings.value.includeNumbers) charset += '0123456789';
    if (settings.value.includeSymbols) charset += '!@#$%^&*()_+-=[]{}|;:,.<>?';
    ...
    generatedPasswords.value = [newEntry, ...generatedPasswords.value.slice(0, 4)];
};
...
const calculateStrength = (pwd) => {
    ...
    if (score < 30) return { level: 'Слабый', color: 'text-red-400', bg: 'bg-red-400' };
    if (score < 60) return { level: 'Средний', color: 'text-yellow-400', bg: 'bg-yellow-400' };
    if (score < 90) return { level: 'Хороший', color: 'text-blue-400', bg: 'bg-blue-400' };
        ...
        <div class="mx-auto max-w-4xl">
            <div class="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p class="text-[#6a6a6b]">Создавайте надежные пароли для защиты ваших аккаунтов</p>
                </div>
                <button
            ...
            <div class="">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <!-- Password Display -->
                    <div class="bg-[#1e1e20] border border-gray-600 rounded-lg p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <button
                                    ...
                                    :disabled="!password"
                                >
                                    <CheckCircleIcon v-if="copied" class="w-5 h-5 text-[#2af16b]" />
                        ...
                        <div v-if="strength" class="mt-4 flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                <span class="text-sm text-[#6a6a6b]">Надежность:</span>
                                <span :class="`text-sm font-medium ${strength.color}`">{{ strength.level }}</span>
                            </div>
                            ...
                            <span class="text-sm text-[#6a6a6b]">{{ password.length }} симв.</span>
                        </div>
                    ...
                    <!-- Settings Panel -->
                    <div v-if="showSettings" class="bg-[#1e1e20] border border-gray-600 rounded-lg p-6">
                        <h3 class="text-lg font-medium mb-4">Параметры генерации</h3>
                        
                        <div class="space-y-4">
                            <div>
                                <label class="flex items-center justify-between mb-2">
                                    <span class="text-sm">Длина пароля: {{ settings.length }}</span>
                                </label>
                                <input
                                    type="range"
                                    min="4"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        v-model="settings.includeUppercase"
                                        class="w-4 h-4 text-[#2af16b] bg-[#0d0d0e] border-gray-600 rounded focus:ring-[#2af16b]"
                                    />
                                    <span class="text-sm">Заглавные буквы (A-Z)</span>
                                </label>
                                
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        v-model="settings.includeLowercase"
                                        class="w-4 h-4 text-[#2af16b] bg-[#0d0d0e] border-gray-600 rounded focus:ring-[#2af16b]"
                                    />
                                    <span class="text-sm">Строчные буквы (a-z)</span>
                                </label>
                                
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        v-model="settings.includeNumbers"
                                        class="w-4 h-4 text-[#2af16b] bg-[#0d0d0e] border-gray-600 rounded focus:ring-[#2af16b]"
                                    />
                                ...
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        v-model="settings.includeSymbols"
                                        class="w-4 h-4 text-[#2af16b] bg-[#0d0d0e] border-gray-600 rounded focus:ring-[#2af16b]"
                                    />
                                ...
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        v-model="settings.excludeSimilar"
                                        class="w-4 h-4 text-[#2af16b] bg-[#0d0d0e] border-gray-600 rounded focus:ring-[#2af16b]"
                                    />
                                    <span class="text-sm">Исключить похожие (i, l, 1, L, o, 0, O)</span>
                                </label>
                                ...
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input
                                        ...
                                        class="w-4 h-4 text-[#2af16b] bg-[#0d0d0e] border-gray-600 rounded focus:ring-[#2af16b]"
                                    />
                                    <span class="text-sm">Исключить неоднозначные</span>
                                </label>
                            </div>
                        </div>
                    </div>
